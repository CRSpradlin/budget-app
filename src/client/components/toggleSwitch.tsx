	...
	public render() {
		return (
			<div className="toggleBox flex flex-col items-start h-6.5 w-12 rounded-full border-2 border-teal-950" onClick={() => this.onClickHandler()}>
                <div className={"togglePin h-6 w-6 rounded-full " + (this.state.value ? "togglePin-toggled bg-teal-400":"togglePin-untoggled bg-teal-800") + (this.props.disabled ? " !bg-slate-500" : "")}>
                </div>
